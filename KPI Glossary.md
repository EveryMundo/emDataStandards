...
| Bookings | Bookings are counted whenever a user completes a flight reservation, also known as the number of transactions. | Number | 1000 |
...
| Load Factor | The number of Revenue Passenger Miles (RPMs) expressed as a percentage of ASMs, either on a particular flight or for the entire system. Load factor represents the proportion of airline output that is consumed. To calculate this figure, divide RPMs by ASMs. The load factor for a single flight can also be calculated by dividing the number of passengers by the number of seats. | Number | 0.5 |
...
| Pageviews | A pageview is recorded when a page is being loaded (or reloaded) in a browser. Pageviews is a metric defined as the total number of pages viewed. | Number | 1000 |
| Passengers | Amount of passengers including adults, children, infants, and seniors in the booking. | Number | 1000 |
| Passenger Yield | Measure of average fare paid per mile, per passenger, calculated by dividing passenger revenue by revenue passenger miles (RPMs). Typically the measure is presented in cents per mile and is a useful measure in assessing changes in fares over time. Yield is not useful for comparisons across markets and/or airlines, as it varies dramatically by the average distance flown and does not incorporate load factor (unlike PRASM) | Number | 1000|
...
| Rev./Impr. | Abbreviation of Revenue per Impression. Total Revenue divided by the number of Impressions shows how much revenue each impression brings. | Number | 3.86 |
| Rev./Booking | Abbreviation of Revenue per Booking. Total Revenue divided by the number of Bookings shows how much, on average, each of the bookings is worth. | Number | 273 |
| Revenue (USD) | Total Revenue in USD | Number | $10,000 |
| ROI | Return on Investment (ROI) is revenue divided by cost | Number | 87.1 |
| RPM | Revenue Passenger Miles (RPMs) is the basic measure of airline passenger traffic. It reflects how many of an airline's available seats were sold. For example, if 200 passengers fly 500 miles on a flight, this generates 100,000 RPMs. | Number | 1000 |
...
|Target CoS| Suggested cost to revenue ratio | Percentage | 50.5% |
